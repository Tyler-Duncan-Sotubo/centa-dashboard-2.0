...
type TiptapEditorProps = {
  value: string; // MUST be HTML string
...
export function RichTextEditor({
  ...
  // ✅ prevents "type one character at a time" by avoiding setContent loops
  ...
  const editor = useEditor({
    ...
    // ✅ Keep parent in sync without causing reset loop
    onUpdate: ({ editor }) => {
  ...
  // ✅ Sync external value ONLY when it truly changed outside the editor
  React.useEffect(() => {
    if (!editor) return;

    const incoming = value || "";

    // If parent is just echoing what we emitted, do nothing
    if (incoming === lastHtmlRef.current) return;

    // If editor already has it, do nothing
    ...
    // Optional: avoid nuking cursor while user is typing
