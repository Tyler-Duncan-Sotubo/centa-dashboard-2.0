...
export const OverviewCards = ({
  ...
  const monthlyAggregates = runSummaries?.reduce(
    (acc, run) => {
    ...
    {} as Record<
      string,
      { totalGross: number; totalBonuses: number; costPerRun: number }
    >,
  );
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 py-6 ">
      {cards.map((c) => (
        <div
          key={c.title}
          className="p-4 rounded-xl shadow-2xs border flex flex-col space-y-3"
        >
